...
Graph.prototype.setPageRanks = function(pageranks) {
	...
	var constant = ((1-.85)/this.names.length)+.85;
	var nodes_in = this.getIn(node).slice();
	var summation = [];
	for (var j = 0; j < this.nodes_in.length; j++) {
	    summation.push(nodes_in[j].getPageRank())/this.getOut(nodes_in[j]).length;
	}
	var sum = summation.reduce(function(a, b) { return a + b });
	node.pagerank = constant + sum;
    }
}
